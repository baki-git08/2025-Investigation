...
* * Control Scheme: Feedback Linearization with a Negative Feedback controller. 
...
/* 
    Rise time = 0.2s 
    Dampling factor = 0.8 
    Percentage Overshoot 1.52% 
*/ 

const float k3 = 433.3653502;    
const float k4 = 33.3078864; 

// --- Control Target --- 
f
loat target_angle = 0.0; // The desired angle for the pendulum (0 = upright) 
...
f
loat armAngle_rad = 0.0;           
// x1: Arm angle 
f
loat armVelocity_rad_s = 0.0;      // x2: Arm angular velocity 
f
loat pendulumAngle_rad = 0.0;      // x3: Pendulum angle 
f
loat pendulumVelocity_rad_s = 0.0; // x4: Pendulum angular velocity 
// Raw encoder positions (updated by interrupts) 
volatile long armEncoderPosition = 0; 
volatile long pendulumEncoderPosition = 0; 
// PID controller variables 
f
loat error_integral = 0.0; 
...
void calculatePendulumSpeed() { 
    ...
    if (timeDelta > 0) { 
        ...
        pendulumVelocity_rad_s = ((float)positionDelta / (float)PPR) * (2.0 * PI) / ((float)timeDelta / 
1000000.0); 
...
void loop() { 
    ...
    /*   SWAPPED PID FOR A NEGATIVE FEEDBACK CONTROL LAW   */ 
    float u_new = -(k3 * pendulumAngle_rad) - (k4 * pendulumVelocity_rad_s); 
